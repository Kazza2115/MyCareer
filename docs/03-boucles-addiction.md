...
| Acte | Âge | Plaisir dominant |
|---|---|---|
| L'ascension | 15-21 | Progression rapide, tout est possible, premiers exploits |
| La conquête | 21-28 | Optimisation, trophées, statut, rivalités au sommet |
| La gestion | 28-33 | Défendre son trône, choix de fin (argent ? légende ? plaisir ?) |
| La sortie | 33+ | Reconversion de poste, dernier défi, tour d'honneur, récit |
...
- **Héritage** (monnaie méta) : gagné selon le score de légende et les quêtes accomplies.
  Se dépense en :
  - **Perks de départ** (max 2 équipés, comme Destiny Eleven — la contrainte est saine) :
    "Mentalité de fer" (+consistance), "Génétique dorée" (−fragilité), "Fils de pro"
    (réputation initiale)…
  - **Origines débloquées** : nouveaux points de départ (pays exotiques, "seconde chance à
    24 ans", gardien reconverti, futsal…) qui changent vraiment la partie.
  - **Cosmétiques** de carte et de profil.
- **Badges** (~150 à collectionner) : exploits précis et savoureux — "Remontada", "Traître"
  (signer chez le rival), "Increvable" (0 blessure en carrière), "Panenka en finale".
  Certains badges sont **secrets** → découverte, wikis communautaires, chasse.
- **Panthéon personnel** : la galerie de toutes tes carrières, avec leurs cartes finales.
  L'effet collection : une carrière ratée y a aussi sa place (et son histoire).
...
- **Défi du jour** : même graine aléatoire pour tout le monde (même joueur de départ, mêmes
  ...
  le rendez-vous quotidien du jeu. **Streak** de participation récompensée (douce : pas de
  perte panique, un streak "gelable").
- **Défi hebdo à contraintes** : "carrière 100 % gardien islandais", "jamais plus de 2 saisons
  au même club" — renouvelle le jeu avec le contenu existant.
- **Panthéon mondial** : classement des meilleures carrières (score de légende) — global,
  par pays, entre amis.
- **Carte de carrière partageable** : image générée (carte finale + palmarès + score) à
  partager en un tap. C'est la boucle d'acquisition : chaque partage est une pub.
- **Ligues entre amis** (V2) : plusieurs joueurs humains dans le **même univers simulé** —
...
- **Récompenses variables** : événements rares et désirables (repérage surprise par un géant,
  ...
  relance aussi pour "peut-être cette fois".
- **Near-miss design** : les échecs racontent ce qui a failli arriver ("Le Real avait envoyé
  un recruteur ce soir-là…"). Un raté doit donner envie de retenter, pas de désinstaller.
- **L'aléa se joue** : la chance existe mais les stats la dominent (cf. doc 02 §8). Le
  joueur doit toujours sentir que *sa* prochaine carrière sera meilleure parce qu'*il* a
  compris quelque chose — pas parce que le dé retombera mieux.
- **Garde-fous éthiques** : pas d'achat de relances, pas de timer artificiel, pas de
