...
#### Le risque gradué : tout est faisable, rien n'est gratuit

Chaque option d'un moment de match a un **niveau de risque** et une **récompense
proportionnelle**. La règle : plus c'est spectaculaire, plus c'est dur — mais jamais
impossible, et la réussite paie à hauteur du risque.
...
| **Frappe de 40 m** | ~5-12 % | **but légendaire : +1.5, une de presse, badge, réputation ++** | −0.4, la presse se moque si tu insistes |
| Panenka en finale | ~35-55 % | légende instantanée | catastrophe médiatique |

\* La probabilité réelle dépend des attributs pertinents (frappe de loin, sang-froid,
...
deux stratégies viables sur une carrière — l'une construit une légende de métronome,
l'autre une légende de showman (avec plus de variance). Le joueur choisit un *style de
risque*, pas entre une bonne et une mauvaise réponse.

Le même principe s'applique aux choix de carrière : signer au géant est la "frappe de
40 mètres" du mercato (improbable de s'y imposer jeune, immense si ça passe), le club
moyen qui te veut vraiment est la "passe décisive" (fiable, moins clinquant). Les deux
routes mènent au sommet.
